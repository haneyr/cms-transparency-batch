...
def load_json_to_bq(table_id, bucket_uri):
    ...
    autodetect=True, source_format=bigquery.SourceFormat.NEWLINE_DELIMITED_JSON, max_bad_records=10000
    )
...
def main():
    ...
    bq_dataset = "cms_datasets" #Change this to reflect your dataset
    bucket_name = "cms-processing-bucket" #Change this to reflect your target bucket for the downloads and split JSON files
    downloadKey = f"{tablename}/downloaded"
    num_file_writers = 70 #  5 for small 70 for big
    queue_size = 1500 # -1 for unlimited
    queue = Queue(queue_size)
    queue_saturation_time = 700  # in seconds 60 for small 1800 for big
